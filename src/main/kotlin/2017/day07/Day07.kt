...
const val INPUT_FILE = "/2017/day07/input.txt"

fun main(args: Array<String>) {
    val input = loadResource(INPUT_FILE)
    val programs = input.lines().filterNot { it.isEmpty() }.map { Program.parse(it) }
...
fun computeTotalWeights(programs: List<Program>): Map<String, Int> {
    ...
    programs.forEach{ totalWeights = computeTotalWeight(it.name, programsByName, totalWeights) }
    return totalWeights
}
...
fun computeTotalWeight(name: String, programs: Map<String, Program>, totalWeights: Map<String, Int>): Map<String, Int> {
    ...
    for (subprogramName in program.subprogramNames) {
        computedTotalWeights = computeTotalWeight(subprogramName, programs, computedTotalWeights)
